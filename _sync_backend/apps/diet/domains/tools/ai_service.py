...
def _guess_image_mime_from_bytes(raw: bytes) -> str:
    """根据魔数判断图片类型，避免 PNG/WebP 被误标为 jpeg。"""
...
def uploaded_image_to_data_url(image_file) -> str:
    """
    构建与真实格式一致的 data URL，供多模态接口使用。
    SiliconFlow 等对「声明为 jpeg 但实际为 png」的 base64 会返回 image_url invalid。
    """
    raw = image_file.read()
    try:
        image_file.seek(0)
...
class AIService:
    ...
    def recognize_food(image_file):
        ...
        """
        # 1. 构建与真实格式一致的 data URL（避免 PNG 误标为 jpeg 导致上游报 invalid image）
        try:
